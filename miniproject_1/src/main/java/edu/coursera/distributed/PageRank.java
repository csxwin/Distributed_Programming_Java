...
public final class PageRank {
     ...
     *
     *   1) JavaPairRDD.join Return an RDD containing all pairs of elements with matching keys in this and other.
     *   2) JavaRDD.flatMapToPair RDD to pairRDD?
     *   3) JavaPairRDD.reduceByKey Merge the values for each key using an associative reduce function.
     *   4) JavaRDD.mapValues apply function to each key, value?
     *
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRank = 
                sites.join(ranks).flatMapToPair(kv -> {
                    ...
                    Integer numberOfOutBound = edges.getNEdges();
                    ...
                    while (iter.hasNext()) {
                        ...
                        contribs.add(new Tuple2(target, 
                            currentRank / (double) numberOfOutBound));
                    }
